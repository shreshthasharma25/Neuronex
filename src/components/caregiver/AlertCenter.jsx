...
export default function AlertCenter() {
  ...
  const getSeverityBadge = (severity, type) => {
    ...
    switch (sev) {
      ...
      case 'INFO':
      default:
        return <span className="px-2.5 py-0.5 rounded-full bg-[#EBF5EE] text-[#1E5E3A] border border-[#D8E2D9] text-[10px] font-extrabold">INFO</span>;
    }
  };
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-[#FFF6E5] border border-[#F3E2C4] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-white text-[#D98A1E] border border-[#F3E2C4] flex items-center justify-center text-xl sm:text-2xl shadow-sm self-start sm:self-auto flex-shrink-0">
          🔔
        </div>
      </div>

      {/* Simulator Tools for Hackathon Demo */}
      <Card variant="white" className="p-4 sm:p-5 border-2 border-dashed border-[#F3E2C4] bg-[#FFF6E5]/50">
      ...
      <div className="space-y-3">
        {alerts.length === 0 ? (
          <div className="p-6 sm:p-8 bg-white rounded-2xl sm:rounded-3xl border border-slate-200 text-center">
            ...
            <h4 className="text-base sm:text-lg font-bold text-[#162832]">All Calm & Clear</h4>
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl flex items-center justify-center text-lg sm:text-xl flex-shrink-0 mt-0.5 ${
                    isWarning ? 'bg-rose-100 text-rose-700' : 'bg-[#EBF5EE] text-[#1E5E3A] border border-[#D8E2D9]'
                  }`}>
                    {isWarning ? '⚠️' : '✓'}
                  </div>
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                      <h4 className={`text-sm sm:text-base font-extrabold ${isWarning ? 'text-rose-900' : 'text-[#1E5E3A]'}`}>
                        {alert.title}
                      </h4>
