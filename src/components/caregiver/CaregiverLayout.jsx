...
export default function CaregiverLayout() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#F3F6F3] relative">
      {/* Caregiver Top Header with Regional Textile trim */}
      <header className="bg-white border-b border-[#D8E2D9] sticky top-0 z-30 shadow-xs">
        <RegionalTextileBorder height={3} />
        <div className="px-3 sm:px-5 py-2.5 sm:py-3 flex items-center justify-between gap-1.5 sm:gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 min-w-0 flex-1">
            <button
              ...
              className="p-2 rounded-xl text-slate-500 hover:text-[#162832] hover:bg-[#EBF5EE] transition-colors sm:hidden flex-shrink-0 touch-target flex items-center justify-center"
            ...
            <div className="hidden sm:block flex-shrink-0">
              <NeuronexLogo variant="icon" size="sm" />
            </div>
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                {selectedPatientId && (
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-[#EBF5EE] hover:bg-[#D8E2D9] text-[#1E5E3A] hover:text-[#164E30] text-xs font-extrabold transition-all flex items-center gap-1.5 border border-[#D8E2D9] shadow-sm"
                ...
                <h1 className="text-sm sm:text-lg font-extrabold text-[#162832] truncate">
                  {selectedPatientId ? preferredName : t('caregiverPortal.title')}
                </h1>
                <span className="px-1.5 sm:px-2 py-0.5 rounded-full bg-[#FFF6E5] text-[#D98A1E] border border-[#F3E2C4] text-[10px] sm:text-xs font-extrabold flex-shrink-0">
                ...
                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                  ...
                  {assignedPatients.length > 1 && (
                    <select
                      ...
                      className="text-[10px] sm:text-[11px] font-extrabold bg-[#EBF5EE] text-[#1E5E3A] border border-[#D8E2D9] rounded-lg px-2 py-0.5 outline-none cursor-pointer"
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            ...
            <button
              ...
              className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#EBF5EE] hover:bg-[#D8E2D9] text-[#1E5E3A] text-xs font-bold border border-[#D8E2D9] transition-all"
            >
            ...
            <button
              ...
              className="flex md:hidden items-center gap-1 px-2 sm:px-2.5 py-1.5 rounded-xl bg-[#FFF6E5] text-[#D98A1E] border border-[#F3E2C4] text-xs font-bold max-w-[110px] sm:max-w-[130px] truncate touch-target"
            ...
            <button
              ...
              className={`flex md:hidden relative p-2.5 rounded-xl border transition-all touch-target items-center justify-center flex-shrink-0 ${
                mobileMenuOpen
                  ...
                  : 'bg-slate-50 hover:bg-[#EBF5EE] text-slate-700 border-slate-200'
              }`}
        ...
        {/* Desktop Tab Strip */}
        <div className="hidden md:flex items-center gap-1 px-3 sm:px-5 py-2 overflow-x-auto no-scrollbar border-t border-[#D8E2D9]/70">
          {tabs.map(tab => {
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap touch-target ${
                  isActive
                    ...
                    : 'text-slate-600 hover:text-slate-900 hover:bg-[#EBF5EE]'
                }`}
              >
                ...
                {tab.badge > 0 && (
                  <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-extrabold ${
                    isActive ? 'bg-white text-[#1E5E3A]' : 'bg-rose-500 text-white'
                  }`}>
                    {tab.badge}
                  </span>
                )}
        ...
        {mobileMenuOpen && (
          <>
            ...
            {/* Menu Drawer Content */}
            <div className="absolute top-full left-0 right-0 z-50 bg-white border-b-2 border-[#D8E2D9] shadow-2xl max-h-[80vh] overflow-y-auto p-3.5 sm:p-4 md:hidden animate-in slide-in-from-top-3 duration-200">
              <div className="flex items-center justify-between pb-2.5 mb-2 border-b border-slate-100">
                <div>
                  <span className="text-[11px] font-extrabold text-slate-400 uppercase tracking-wider block">
                    Caregiver Portal Menu
                  </span>
                  <span className="text-xs font-bold text-[#162832]">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs sm:text-sm font-extrabold transition-all min-h-[46px] touch-target ${
                        isActive
                          ? 'bg-[#1E5E3A] text-white shadow-sm'
                          : 'text-slate-700 hover:bg-[#EBF5EE] active:bg-[#D8E2D9]'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                          isActive ? 'bg-white/20 text-white' : 'bg-[#EBF5EE] text-[#1E5E3A]'
                        }`}>
                          <Icon className="w-4 h-4" />
                        </div>
                        <span>{tab.label}</span>
              ...
              <div className="mt-3.5 pt-3 border-t border-slate-100 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-[#EBF5EE] hover:bg-[#D8E2D9] text-[#1E5E3A] text-xs font-extrabold border border-[#D8E2D9] transition-all min-h-[42px] touch-target"
                  >
                    <span>👨‍👩‍👧</span>
                    <span>Family</span>
                  </button>
