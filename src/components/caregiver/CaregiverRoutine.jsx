    ...
    <div className="space-y-6">
      ...
      {routine.length === 0 ? (
        <Card variant="white" className="p-6 sm:p-8 text-center border-dashed border-2 border-[#D8E2D9]">
          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-[#EBF5EE] text-[#1E5E3A] flex items-center justify-center text-2xl sm:text-3xl mx-auto mb-3">
            ⏰
          </div>
        ...
        <div className="relative pl-5 sm:pl-6 space-y-3 sm:space-y-4 before:absolute before:left-2.5 sm:before:left-3 before:top-3 before:bottom-3 before:w-0.5 before:bg-slate-200">
          {routine.map((item, idx) => (
            <div
              key={item.id || idx}
              className="relative p-3.5 sm:p-4 rounded-xl sm:rounded-2xl bg-white border border-[#D8E2D9] shadow-sm flex items-center justify-between gap-2.5 sm:gap-3"
            >
              {/* Milestone Dot */}
              <div className="absolute -left-5 sm:-left-6 top-4 sm:top-5 w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full bg-[#1E5E3A] border-2 border-white shadow-xs" />

              <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl bg-[#EBF5EE] text-[#1E5E3A] font-extrabold text-[11px] sm:text-xs flex items-center gap-1 whitespace-nowrap flex-shrink-0">
                  <Clock className="w-3.5 h-3.5" />
                  <span>{item.time}</span>
                </div>
                <h4 className="text-xs sm:text-base font-extrabold text-[#162832] truncate">
                  {item.action}
                </h4>
              </div>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button
                  ...
                  className="p-1.5 sm:p-2 rounded-xl text-slate-400 hover:text-[#1E5E3A] hover:bg-[#EBF5EE] transition-colors touch-target flex items-center justify-center"
                  title="Edit step"
                >
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm font-medium focus:border-[#1E5E3A] focus:outline-none"
            />
          </div>
